...
6、分页显示一个文件或任何输出结果：more
	描述：
	分页显示一个文件或任何输出结果用于查看纯文本文件(较长的)格式
	格式：more[选项] 文件
...
8、显示前几行文件内容：head
	head[必要参数][选择参数][文件]
	命令参数：-n 10 显示前10行 -n -10 正常输出但不显示最后的10行
	eg：显示new.txt的前两行内容
	head -n 2 new.txt head -2 new.txt

9、指定显示文件后多少行内容：tail 
	命令参数：-n 10 显示后面10行 -f 持续刷新显示的内容 eg：显示new.txt的末尾两行内容
	tail -n 2 new.txt tail -2 new.txt eg：指定从第二行开始显示 tail -n +2 new.txt

10、查看一个文件：cat 用于查看纯文本文件 用法：cat catalina.out

11、查看文件内容多少字符多少行多少字节：wc 用法：wc catalina.out

12、排序文件内容：sort 用法：sort [选项] [文件]eg：sort -b h.txt
13、	
